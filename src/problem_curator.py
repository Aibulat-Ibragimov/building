...
try:
    ...
    problems = {
        item['code']: item['name'] for item in data['problems']
    }

    # Извлекаем записи из блока с проблемами
    curators = data['table_oks_with_problems'][0]['curators']
    ...
    for curator in curators:
        if curator['curator_id'] == target_curator_id:
            for curator_record in curator['curator_records']:
                for problem in curator_record['problems']:
                    row = {
                        ...
                        "date_detected_problem": problem['date_detected_problem']
                    }
                    rows.append(row)
    ...
    df['risk_name'] = df['problem_code'].map(risks)
    df['problem_name'] = df['problem_code'].map(problems)
    ...
    wb.save(f"C:\\Users\\Aibulat\\work\\building\\output\\oks_with_problems_{target_curator_id}_{date_now}.xlsx")

    print(f"Данные успешно сохранены в файл oks_with_problems_{target_curator_id}_{date_now}.xlsx")
